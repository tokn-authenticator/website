...
function renderAnswer(text: string) {
  return text.split("__EMAIL__").map((seg, si, arr) => (
    <Fragment key={si}>
      {renderTokens(seg)}
        ...
        {faqs.map((f) => (
          <details key={f.id} className="group">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5 text-left sm:p-6 [&::-webkit-details-marker]:hidden">
              <span className="font-medium">{t(`${f.id}.q`)}</span>
              <ChevronDown className="h-5 w-5 shrink-0 text-muted transition-transform duration-200 group-open:rotate-180" />
            </summary>
            <p className="px-5 pb-6 text-pretty leading-relaxed text-muted sm:px-6">{renderAnswer(t(`${f.id}.a`))}</p>
          </details>
        ))}
      </div>
    </section>
